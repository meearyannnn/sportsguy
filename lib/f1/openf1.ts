...
async function fetchOpenF1<T>(endpoint: string, ttl = CACHE_TTL_LIVE_MS): Promise<T> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Telemetry sensor error: ${res.status}`);
    }
...
export async function getSessionWeather(sessionKey: number | string): Promise<OpenF1Weather | null> {
  try {
    ...
    return weather && weather.length > 0 ? weather[weather.length - 1] : null;
  } catch (err) {
...
export async function getCarTelemetry(
  ...
  try {
    ...
    return carData && carData.length > 0 ? carData.slice(-25) : [];
  } catch (err) {
