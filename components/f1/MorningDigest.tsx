...
export default function MorningDigest({
  ...
  // Derive top 3 or fallback
  const top3 = results.slice(0, 3).map((r, i) => ({
    pos: r.position || String(i + 1),
    driver: `${r.Driver?.givenName || ''} ${r.Driver?.familyName || ''}`.trim() || ['Max Verstappen', 'Lando Norris', 'Lewis Hamilton'][i],
    team: r.Constructor?.name || ['Red Bull Racing', 'McLaren', 'Ferrari'][i],
    gap: i === 0 ? 'WINNER' : r.Time?.time || '+2.214s',
  }));

  return (
    <div className="rounded-sm border border-[var(--border-subtle)] bg-[var(--bg-secondary)] overflow-hidden transition-all">
      {/* Header Bar */}
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[var(--border-subtle)] bg-[var(--bg-tertiary)]/50">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-sm bg-[var(--accent-f1-red)]/10 border border-[var(--accent-f1-red)]/30 flex items-center justify-center text-[var(--accent-f1-red)] shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono-num px-1.5 py-0.2 rounded-sm bg-emerald-950/40 text-emerald-400 border border-emerald-800/40">
                OFFICIAL CLASSIFICATION
              </span>
            </div>
            <h4 className="font-hud font-black text-base uppercase text-[var(--text-primary)]">
              {raceName} • Race Summary & Podium
            </h4>
          </div>
        </div>
        ...
        {onViewResults && (
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-sm bg-[var(--bg-tertiary)] hover:bg-[var(--border-hover)] border border-[var(--border-subtle)] text-xs font-hud font-bold uppercase tracking-wider text-[var(--text-primary)] transition-all cursor-pointer self-start sm:self-auto"
          >
      ...
      <div className="p-4 sm:p-5 space-y-4">
        <div className="text-xs text-[var(--text-secondary)] leading-relaxed">
          ...
          A high-speed tactical chess match concluded at {circuitName}. Heavy tire degradation across
          the second stint and varying 1-stop vs 2-stop strategies produced a dramatic finish in the final 8 laps.
        </div>

        {/* Podium Area (Directly Visible) */}
        <div className="rounded-sm border border-[var(--border-subtle)] p-4 bg-[var(--bg-primary)] space-y-2.5">
